...
socket.addEventListener('message', (event) => {
  ...
  console.log(
    'Received message from server: ' + messageObject.type
  )
  switch (messageObject.type) {
    ...
    default:
      console.error(
        'Unknown message type: ' + messageObject.type
      )
  }
})
...
function showMessage(message) {
  ...
  if (message.user.id === userId) {
    ...
    innerMessageElement.classList
      .add('bg-green-500', 'border-2', 'border-green-400')
  } else {
    innerMessageElement.classList
      .add('bg-slate-500', 'border-2', 'border-gray-400')
  }
